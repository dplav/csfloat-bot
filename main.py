...
def is_good_deal(name, price_eur, wear):
    # Comparaison souple pour ignorer les problèmes de caractères spéciaux
    if "Ultraviolet" in name and "Field-Tested" in name:
        return price_eur <= 565.50 and wear <= 0.2409

    if "Stained" in name:
        return price_eur <= 550.50
...
def get_market_data():
    ...
    # On utilise le TYPE exact pour butterfly et on trie manuellement en local
    # C'est la seule méthode qui garantit de voir TOUS les modèles peu importe le nom
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for i in items:
                ...
                # On récupère le nom tel qu'enregistré sur les serveurs Steam
...
def main():
    ...
    print("🚀 Sniper v34.0 (Formatage Précision)")
    
    # Message de démarrage
    r = requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage", 
                      json={"chat_id": TELEGRAM_CHAT_ID, "text": "🔄 Initialisation avec détection textuelle précise..."})
    ...
    while True:
        ...
        report = (f"🖥️ *DASHBOARD SNIPER BFK*\n"
                  ...
                  f"    └ En ligne (Top 100) : `{uv_t}` | Deals : `{len(uv_d)}` \n\n"
                  f"🔵 *Stained (Max 550€)*\n"
                  f"    └ En ligne (Top 100) : `{st_t}` | Deals : `{len(st_d)}` \n\n"
                  f"🛰️ *Status :* `Scan 100 items Butterfly` ")
